...
class TestWorker(RQTestCase):
    ...
    def test_create_worker(self):
        ...
        bar = Queue('bar', storage=self.conn)
        ...
        w = Worker(['foo', 'bar'], storage=self.conn)
        self.assertEqual(w.queues[0].name, 'foo')
        self.assertEqual(w.queues[1].name, 'bar')

        # With iterable of strings
        w = Worker(iter(['foo', 'bar']), storage=self.conn)
        self.assertEqual(w.queues[0].name, 'foo')
        self.assertEqual(w.queues[1].name, 'bar')
        ...
        if PY2:
            ...
            w = Worker(b'foo', storage=self.conn)
            self.assertEqual(w.queues[0].name, 'foo')

            # With list of byte strings
            w = Worker([b'foo', b'bar'], storage=self.conn)
            self.assertEqual(w.queues[0].name, 'foo')
            self.assertEqual(w.queues[1].name, 'bar')

            # With iterable of byte strings
            w = Worker(iter([b'foo', b'bar']), storage=self.conn)
            self.assertEqual(w.queues[0].name, 'foo')
            self.assertEqual(w.queues[1].name, 'bar')
    ...
    def test_work_and_quit(self):
        ...
        w = Worker([fooq, barq], storage=self.conn)
    ...
    def test_work_is_unreadable(self):
        ...
        job = self.create_job(func=div_by_zero, args=(3,))
        job.save()
    ...
    def test_custom_exc_handling(self):
        ...
        w = Worker([q], exception_handlers=black_hole, storage=self.conn)
        w.work(burst=True)  # should silently pass
    ...
    def test_get_current_job(self):
        ...
        q = Queue(storage=self.conn)
        worker = Worker([q], storage=self.conn)
    ...
    def test_custom_job_class(self):
        ...
        q = Queue(storage=self.conn)
        worker = Worker([q], job_class=CustomJob, storage=self.conn)
        self.assertEqual(worker.job_class, CustomJob)

    def test_custom_queue_class(self):
        """Ensure Worker accepts custom queue class."""
        q = CustomQueue(storage=self.conn)
        worker = Worker([q], queue_class=CustomQueue, storage=self.conn)
    ...
    def test_custom_queue_class_is_not_global(self):
        ...
        q = CustomQueue(storage=self.conn)
        worker_custom = Worker([q], queue_class=CustomQueue, storage=self.conn)
        q_generic = Queue(storage=self.conn)
    ...
    def test_custom_job_class_is_not_global(self):
        ...
        q = Queue(storage=self.conn)
        worker_custom = Worker([q], job_class=CustomJob, storage=self.conn)
        q_generic = Queue(storage=self.conn)
        worker_generic = Worker([q_generic], storage=self.conn)
    ...
    def test_prepare_job_execution(self):
        ...
        queue = Queue(storage=self.testconn)
        job = queue.enqueue(say_hello)
        worker = Worker([queue], storage=self.conn)
        worker.prepare_job_execution(job)

        # Updates working queue
        registry = StartedJobRegistry(storage=self.testconn)
    ...
    def test_clean_queue_registries(self):
        ...
        foo_queue = Queue('foo', storage=self.testconn)
        foo_registry = StartedJobRegistry('foo', storage=self.testconn)
        self.testconn.zadd(foo_registry.key, 1, 'foo')
        self.assertEqual(self.testconn.zcard(foo_registry.key), 1)

        bar_queue = Queue('bar', storage=self.testconn)
        bar_registry = StartedJobRegistry('bar', storage=self.testconn)
        self.testconn.zadd(bar_registry.key, 1, 'bar')
        self.assertEqual(self.testconn.zcard(bar_registry.key), 1)

        worker = Worker([foo_queue, bar_queue], storage=self.conn)
    ...
    def test_worker_calls_clean_registries(self):
        ...
        worker = Worker(queue, storage=self.testconn)
